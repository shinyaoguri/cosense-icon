...
| `date` | `target`, `基準日` | (なし) | `countdown` / `countup` の基準日 (`YYYY-MM-DD`) |
...
パスに `/countdown/` または `/countup/` セグメントを含めると、`date` オプションで指定した
**基準日までの残り日数 / 経過日数**を、テキスト中のプレースホルダ `{d}` に埋め込む。
`today` 等と同じくアクセス時点で計算され、キャッシュされない (`no-store`)。

```
/countdown/date-2026-12-31/締切まであと{d}日.svg   # 「締切まであと165日」
/countup/date-2026-04-01/プロジェクト{d}日目.svg    # 「プロジェクト109日目」
/countdown/date-2025-12-25/{d}.svg                  # 数字だけ
/bg-1e40af/fg-fff/radius-24/countdown/date-2026-12-31/あと{d}日.svg   # 装飾と併用
```

- **数え方**: 当日 = `0`。countdown は基準日を過ぎても `0` で下げ止め、countup は基準日前は `0`。
  - countdown = `max(0, 基準日 − 今日)` / countup = `max(0, 今日 − 基準日)`
- **プレースホルダ**: `{d}` を主とし、`{}` / `{n}` / `{days}` も同じく日数へ置換 (複数出現も全て)。
  1 つも書かなければ末尾にスペース区切りで数値を付加する。
- **既存機能と併用可**: 置換後は通常テキストとして描画されるので、`bg`/`fg`/`size`/`font`/`radius`/
...
Google Fonts を選んだ場合、**プレビューはその場でブラウザが組む** (日数を埋め、配信時と同じ
グリフパック経由で描く)。R2 への登録はコピー / ダウンロードのボタンを押したときだけなので、
登録前から実際のフォントで確認できる。ここで組んだパックはキャッシュされ、ボタンを押したときに
再利用される。
